# app.py (финальная версия для OpenRouter)
...
import ast
from classifier.model_or import classify_with_openrouter   # предполагаем, что там правильная функция
...
openrouter_api_key = st.text_input("OpenRouter API Key", type="password",
                                   value="", key="openrouter_key_input",
...
if not gigachat_api_key:
    ...
    if prompt := st.chat_input("Введите сообщение..."):
        ...
        with st.spinner("Оценка ответа классификатором (OpenRouter)..."):
            ...
            label, reason = classify_with_openrouter(prompt, full_response, similar, api_key=openrouter_api_key, context=context)
